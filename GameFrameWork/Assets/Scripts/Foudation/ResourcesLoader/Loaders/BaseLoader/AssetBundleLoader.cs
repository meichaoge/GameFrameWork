    ...
    public class AssetBundleLoader : BaseAbstracResourceLoader
    {
        ...
        protected virtual IEnumerator LoadDepdenceAssetBundleAsync(string url, AssetBundleLoader depdebceAssetBundleLoader)
        {
            ...
            for (int dex = 0; dex < allDependenceAssetLoader.Count; ++dex)
            {
                ...
                {
                    Debug.LogError("[AssetBundleLoader] LoadAssetBundleSync  Fail, 依赖的 AssetBundle 资源不存在 " + allDependenceAssetLoader[dex].m_ResourcesUrl);
                    OnCompleteLoadDepdenceAsset(false, string.Format("[AssetBundleLoader] LoadAssetBundle Fail,AssetBundle Path={0}" + allDependenceAssetLoader[dex].m_ResourcesUrl), null, true);
                    yield break;
                }
            } //判断依赖项是否加载完成
            ...
            if (wwwLoader.IsError)
            {
                OnCompleteLoadDepdenceAsset(false, string.Format("[AssetBundleLoader] LoadAssetBundle Fail, AssetBundle Path={0}" + (S_AssetBundleTopPath + depdebceAssetBundleLoader.m_ResourcesUrl)), null, true);
                WWWLoader.UnLoadAsset(wwwLoader.m_ResourcesUrl);
                yield break;
            }

            ResultObj = (wwwLoader.ResultObj as WWW).assetBundle;//.LoadAsset(System.IO.Path.GetFileNameWithoutExtension(assetName));
            OnCompleteLoadDepdenceAsset(ResultObj == null, string.Format("[AssetBundleLoader] LoadAssetBundleSuccess " + depdebceAssetBundleLoader.m_ResourcesUrl), ResultObj, true);
            WWWLoader.UnLoadAsset(wwwLoader.m_ResourcesUrl);
            #endregion
